...
const DeleteArtistModal = ({ artist, isOpen, onClose, onSuccess }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                addToast({
                    description: language === 'heb' ? 'האמן נמחק בהצלחה' : 'Artist deleted successfully',
                    color: "success",
                    timeout: 3000
                });
                onSuccess();
                ...
                addToast({
                    description: error.message || (language === 'heb' ? 'שגיאה במחיקת האמן' : 'Error deleting artist'),
                    color: "danger",
                    timeout: 3000
                });
            }
            ...
            addToast({
                description: language === 'heb' ? 'שגיאה במחיקת האמן' : 'Error deleting artist',
                color: "danger",
                timeout: 3000
            });
        } finally {
    ...
    const translations = {
        deleteArtist: { heb: 'מחק אמן', eng: 'Delete Artist' },
        confirmDelete: { heb: 'האם אתה בטוח שברצונך למחוק את האמן', eng: 'Are you sure you want to delete the artist' },
        ...
        <Modal isOpen={isOpen} onClose={handleClose}>
            <ModalContent style={{ direction: language === 'heb' ? 'rtl' : 'ltr' }}>
                <ModalHeader style={{ textAlign: language === 'heb' ? 'right' : 'left' }}>
                    {translations.deleteArtist[language]}
                </ModalHeader>
                <ModalBody>
                    <div className="space-y-4" style={{ direction: language === 'heb' ? 'rtl' : 'ltr' }}>
                        <div className="text-base" style={{ textAlign: language === 'heb' ? 'right' : 'left' }}>
                            {translations.confirmDelete[language]} <strong>{artist?.name?.heb} / {artist?.name?.eng}</strong>?
                        </div>
